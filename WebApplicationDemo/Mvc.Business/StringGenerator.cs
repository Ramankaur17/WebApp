    ...
    public class StringGenerator : IStringGenerator
    {
        public string Format(string number)
        {
            ...
            try
            {
                ...
                {
                    return (isNegative + ConvertToWords(number)).ToUpper();
                }
        ...
        private string ConvertToWords(string numb)
        {
            ...
            try
            {
                ...
                if (decimalPlace > 0)
                {
                    ...
                    {
                        andStr = wholeNo != "1" ? wholeNo == "0" ? string.Empty : "dollars and" : "dollar and";// just to separate whole numbers from points/cents  
                        endStr = string.Format("{0} {1}", points != "1" ? "cents" : "cent", endStr);//Cents  
                        pointStr = ConvertDecimals(points);
                    }
                }
                val = string.Format("{0} {1}{2} {3}", ConvertWholeNumber(wholeNo).Trim(), andStr, pointStr, endStr).Trim();
            }
            catch { }
            return val;
        }
        ...
        {
            string cd = string.Empty, digit = string.Empty, engOne = string.Empty;
